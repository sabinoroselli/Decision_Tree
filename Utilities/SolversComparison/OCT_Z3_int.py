...
def optimal_DT(df,Splits):
    ...
    z = { (i,t):Int(f'z_{i}_{t}') for t in T_L for i in I} # point 'i' is in node 't'
    ...
    if s.check() == sat:
        ...
        splitting_nodes = {
            i: {
                ...
                'b': float(''.join(solution[f'b_{i}'].as_decimal(6)[:-1]))
                                if str(solution[f'b_{i}']) != '0' else 0
            }
            for i in T_B if solution[f'd_{i}'].as_long() == 1
        }
