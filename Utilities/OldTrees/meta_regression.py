...
from TreeStructure import OptimalTree,Parent,RRSE,RAE
...
def optimal_RMT(df, features, labels, Splits, C, config ):
    ...
    if config['Meta']:
        features1 = features[0]  # for branch nodes
        features2 = features[1]  # for leaf nodes
    else:
        features1 = features
        features2 = features
    ...
    m.setParam('TimeLimit', config['Timeout'] * 60)

    # variables
    d = m.addVars(T_B,vtype=GRB.BINARY,name='d') # d_t = 1 if node splits
    if config["SplitType"] == "Parallel":
        a = m.addVars(features1, T_B, lb=0, ub=1, vtype=GRB.INTEGER, name='a')
    elif config["SplitType"] == "Oblique":
        a = m.addVars(features1, T_B, lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name='a')
        a_abs = m.addVars(features1, T_B, vtype=GRB.CONTINUOUS, name='a_abs')
        s = m.addVars(features1, T_B,lb=0,ub=1, vtype=GRB.INTEGER, name='s')
    ...
    Beta = m.addVars(features2,T_L,lb=-GRB.INFINITY, vtype=GRB.CONTINUOUS, name='Beta')  # coefficient for feature i at node t
    Bet_abs = m.addVars(features2,T_L, vtype=GRB.CONTINUOUS, name='Beta_abs')  # coefficient for feature i at node t
    ...
    if config["SplitType"] == "Parallel":
        Const_1 = m.addConstrs(
            quicksum([a[j, t] for j in features1]) == d[t] for t in T_B
        )
    elif config["SplitType"] == "Oblique":
        Const_0 = m.addConstrs(
            a_abs[j, t] == abs_(a[j, t]) for j in features1 for t in T_B
        )

        Const_01 = m.addConstrs(
            s[j, t] >= a_abs[j, t] for j in features1 for t in T_B
        )
        # Guarantee that the sum of fractions for the split is equal to 1
        Const_1 = m.addConstrs(
            quicksum([a_abs[j, t] for j in features1]) <= d[t] for t in T_B
        )

        Const_11 = m.addConstrs(
            s[j, t] <= d[t] for j in features1 for t in T_B
        )

        Const12 = m.addConstrs(
            quicksum([s[j, t] for j in features1]) >= d[t] for t in T_B
        )
    ...
    if config["SplitType"] == "Parallel":
        Const_12 = m.addConstrs(
            (z[i, l] == 1)
            >>
            (quicksum([a[j, t] * (df.loc[i, j] + mu[j] - mu_min) for j in features1]) + mu_min <= b[t])
        ...
        Const_12 = m.addConstrs(
            (z[i, l] == 1)
            >>
            (quicksum([a[j, t] * df.loc[i, j] for j in features1]) + 0.0001 <= b[t])
            # + (1-z[i,l]) * (bigM[i] + mu_max)
            for i in I
            for l in T_L
            for t in A_l[l]
    ...
    Const_13 = m.addConstrs(
        (z[i, l] == 1)
        >>
        (quicksum([a[j, t] * df.loc[i, j] for j in features1]) >= b[t]) # - (1 - z[i,l]) * bigM[i]
        for i in I
        for l in T_L
        for t in A_r[l]
    )
    ...
    Const_17 = m.addConstrs(
        (z[i,t] == 1)
        >>
        (quicksum([Beta[j, t] * df.loc[i, j] for j in features2]) + Delta[t] - df.loc[i,labels[0]] == gamma[i, t] )
        for i in I
        for t in T_L
    )
    ...
    Const_19 = m.addConstrs(
        Bet_abs[f, t] == abs_(Beta[f, t]) for f in features2 for t in T_L
    )
    ...
    bounds = {
        i:[df[i].min(),df[i].max()] for i in features1
    }

    Const_200 = m.addConstrs(
        (a[j,t] == 1) >> (b[t] <= bounds[j][1])
        for t in T_B for j in features1
    )
    ...
    m.setObjective(
        # variance cost
        quicksum([ Bet_abs[f,t] for f in features2 for t in T_L ]) + C * quicksum([ gamm_abs[i,t] for i in I for t in T_L])
        )
    ...
    if m.status != GRB.INFEASIBLE:
        ...
        if config["SplitType"] == "Parallel":
            splitting_nodes = {
                i: {
                    'a': [f for f in features1 if solution[f'a[{f},{i}]'] > 0][0],
...
if __name__ == "__main__":
    ...
    df = DataParser(f'{file}.arff','Regression', one_hot=False)
    ...
    features = [config['Branch_feat'], config['Leaf_feat']]
    ...
    train_pred = ODT.predict_regr(X_train, the_tree,features[1])
    ...
    test_pred = ODT.predict_regr(X_test, the_tree,features[1])
