...
def SRTL_DL85(file, mode, RS, ProblemType):
    ...
    if ProblemType == 'Regression':
        if mode == 'SRTL':
            clf = STreeDPiecewiseLinearRegressor(max_depth=10,max_num_nodes=20,random_seed=RS)
        # elif mode == 'DL85':
        #     clf = DL85Predictor()
        # elif mode == 'SVM':
    ...
    for m in methods:
        for i in choice[0]:
            ...
            for RS in range(Runs):
                Metric,Leaves = SRTL_DL85(i, m, RS, choice[1])
                for key,value in Metric.items():
